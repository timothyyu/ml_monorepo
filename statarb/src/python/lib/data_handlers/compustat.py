...
def __getGlobalCoids():
    rows=database.execute("SELECT coid FROM {} GROUP BY coid HAVING SUM(issueid NOT LIKE '%W')=0".format(database.STOCK_TABLE)).fetchall()
    for row in rows:
        __globalCoids.add(row["coid"])
        
#Filter which secids and coids are populated
def __globalCompanyFilter(coid):
#    if coid in __globalCoids:
#        return True
#    else:
#        return False
    return True

def __globalSecidFilter(secid):
#    if secid is None:
#        return False
#    
...
def __localCompanyFilter(coid):
    return True

def __localSecidFilter(secid):
    return True
...
def __processSecurity(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid):
...
def __processCSHOC(command,keys,attributes,timestamp,source,backfill,processCoid,processSecid):
...
def __processPrice(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid):
    secid = database.getSecidFromCsid(keys["GVKEY"], keys["IID"], timestamp)
    if not processSecid(secid):
        return
...
def __processDividend(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid):
    secid = database.getSecidFromCsid(keys["GVKEY"], keys["IID"], timestamp)
    if not processSecid(secid):
        return
...
def __processSplit(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid):
...
def __processCompany(command, keys, attributes, timestamp,source,backfill,processCoid,processSecid):
    date=0L
    coid=int(keys["GVKEY"])
    ...
    for n,v in attributes.iteritems():
        if n not in ('GSECTOR', 'GGROUP', 'GIND', 'GSUBIND', 'NAICS', 'SIC', 'SPCSECCD', 'SPCINDCD', 'FIC', 'FYRC', 'ACCTSTD', 'FYR', 'ISMOD', 'SRC', 'UPD'):
            continue
        if command in ("C","I"):
...
def __processIndustry(command, keys, attributes, timestamp,source,backfill,processCoid,processSecid):
    date=util.convert_date_to_millis(keys["DATADATE"])
    if backfill: 
        timestamp=util.convert_date_to_millis(__datePlusOne(keys["DATADATE"]))
    coid=int(keys["GVKEY"])
...
def __processCredit(command, keys, attributes, timestamp,source,backfill,processCoid,processSecid):
    date=util.convert_date_to_millis(keys["DATADATE"])
    if backfill: 
        timestamp=util.convert_date_to_millis(__datePlusOne(keys["DATADATE"]))
    coid=int(keys["GVKEY"])
...
def __processHgic(command, keys, attributes, timestamp,source,backfill,processCoid,processSecid):
...
def __processFundamental(command, keys, attributes, timestamp,source,backfill,processCoid,processSecid):
...
def __processDesind(command, keys, attributes, timestamp,source,backfill,processCoid,processSecid):
    if keys['INDFMT'] != 'INDL' or keys['DATAFMT'] != "STD":
        return
...
def __processMkt(command, keys, attributes, timestamp,source,backfill,processCoid,processSecid):
    if keys['CFFLAG'] != 'F':
        return
...
def __processFiledate(command, keys, attributes, timestamp,source,backfill,processCoid,processSecid):
...
def process(filepath, source):
    ...
    while True:
        ...
        if table == "security":
            __processSecurity(command, keys, attributes, timestamp, source,backfill,processCoid,processSecid)
        elif table == "sec_dprc":
            __processPrice(command, keys, attributes, timestamp, source,backfill,processCoid,processSecid)
            __processCSHOC(command, keys, attributes, timestamp, source,backfill,processCoid,processSecid)
        elif table=="company":
            __processCompany(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table == "sec_divid":
            __processDividend(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table=="sec_split":
            __processSplit(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table=="co_industry":
            __processIndustry(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table == "co_hgic":
            __processHgic(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table in ("co_afnd1","co_afnd2","co_ifndq", "co_ifndsa", "co_ifndytd"):
            __processFundamental(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table in ("co_idesind", 'co_adesind'):
            __processDesind(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table in ( "co_amkt",'co_imkt'):
            __processMkt(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table == "co_filedate":
            __processFiledate(command, keys, attributes, timestamp, source, backfill,processCoid,processSecid)
        elif table == "adsprate":
            __processCredit(command, keys, attributes, timestamp, source, backfill, processCoid, processSecid)
        elif table == "exrt_dly":
            __processExchange(command, keys, attributes, timestamp, source, backfill)
        else:
            continue
