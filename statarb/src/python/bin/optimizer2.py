...
def __printpointinfo(name,target):
    ...
    utility4 = costsFunc(positions, target, borrowRate)
    print "@{}: mu={:.0f}, risk={:.0f}, slip={:.0f}, costs={:.2f}, ratio={:.3f}".format(name,utility1,utility2,utility3,utility4,utility1/utility2)

def objective(target, kappa, slippage, positions, mu, rvar, factors, fcov, adv, brate):
    ...
    utility -= costsFunc(positions, target, brate)
...
def costsFunc(positions, target, brate):
    zeros = numpy.zeros(len(target))
    return brate * (numpy.minimum(zeros, positions) - numpy.minimum(zeros, target)).sum() 

def objective_grad(target, kappa, slippage, positions, mu, rvar, factors, fcov, advp, brate):
    F = factors
    Ft = numpy.transpose(F)   
    return mu - 2 * kappa * (rvar * target + numpy.dot(Ft, numpy.dot(fcov, numpy.dot(F, target)))) - slippage * numpy.sign(target - positions)
...
def optimize(daily):
    ...
    p.args.f = (kappa, slippage, positions, mu, rvar, factors, fcov, advp, borrowRate)
...
if __name__ == "__main__":
    ...
    daily = True
    (target, dutil, eslip, dmu, costs) = optimize(daily)
    
    for ii in range(len(target)):
        print "T|{}|{}|{}|{}|{}|{}|{}".format(ii, sec_ind_rev[ii], target[ii], dutil[ii], dmu[ii], eslip[ii], costs[ii])
